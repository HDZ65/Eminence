        ...
        <div>
            ...
            <main className="container flex flex-col gap-12 md:gap-16 lg:gap-20 mx-auto px-4 md:px-6 lg:px-20 py-12 md:py-20">
                <section className="flex flex-col md:flex-row gap-12 lg:gap-16">
                    <p className="w-full md:w-1/2 text-xl">&quot;La maison Éminence présente sa collection de joaillerie réalisée en or. La collection s&apos;appuie sur des pratiques durable qui caratérisent le luxe moderne, utilisant des techniques ancestrales et des matériaux nobles.&quot;</p>
                </section>

                <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-12 lg:gap-8">
                    {images.map((image, index) => (
                        <div key={index} className={`relative group ${image.className}`}>
                            <Link href={`/collection-joaillerie/${image.title}`}>
                                <div className="overflow-hidden h-full w-full">
                                    <Image src={image.src} height={1000} width={1000} alt={image.alt} className={`${image.className} group-hover:brightness-75 group-hover:scale-110 transition-all duration-500`} />
                                </div>
